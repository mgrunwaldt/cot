<?php
	...
	function createXMLPHP5($arreglo){
		...
		foreach($arreglo as $key => $value){
			...
			else if(ereg('tax_([0-9]{1}|[0-9]{2})_name',$key)){

				$keyam = ereg_replace('(^tax_)|(_name$)','',$key);

				$taxesName[$keyam] = $value;

				//array_push($taxesName,array($keyam => $value));
			}else if(ereg('tax_([0-9]{1}|[0-9]{2})_amount',$key)){

				$keyam = ereg_replace('(^tax_)|(_amount$)','',$key);
	...
	function BASE64URLRSA_encrypt ($valor,$publickey) {
		...
		if($resp){
			...
			$base64 = ereg_replace('(/)','_',$base64);
			$base64 = ereg_replace('(\+)','-',$base64);
			$base64 = ereg_replace('(=)','.',$base64);
	...
	function BASE64URLRSA_decrypt($valor,$privatekey){
		...
		$pas = ereg_replace('_','/',$valor);
		$pas = ereg_replace('-','+',$pas);
		$pas = ereg_replace('\.','=',$pas);
	...
	function BASE64URL_symmetric_cipher($dato, $key, $vector)
	{
		...
		$crypttext = ereg_replace('(/)','_',$crypttext);
		$crypttext = ereg_replace('(\+)','-',$crypttext);
		$crypttext = ereg_replace('(=)','.',$crypttext);
	...
	//-------------------------------------------------------------------------------------
 	...
 	function BASE64URL_digital_generate($dato, $privatekey)
 	{
		...
		if($resp){
			...
			$crypttext = ereg_replace('(/)' ,'_',$base64);
			$crypttext = ereg_replace('(\+)','-',$crypttext);
			$crypttext = ereg_replace('(=)' ,'.',$crypttext);
