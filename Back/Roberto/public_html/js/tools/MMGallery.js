...
MMGallery.newGallery = function (divId, files, zoom) {
    ...
    auxGallery.showNextImage = MMGallery.showNextImage;
    auxGallery.showPreviousImage = MMGallery.showPreviousImage;
    auxGallery.showAmplifiedImage = MMGallery.showAmplifiedImage;
...
//    });
    ...
    $("#MMFiles").on('click', '.MMFilesItemImage', function () {
        var src = $(this).attr("src");
        instance.showAmplifiedImage(src);
    }
    );
    ...
    $("#rightArrowGallery").on('click', function () {
        instance.showNextImage();
    }
    );
    $("#leftArrowGallery").on('click', function () {
        instance.showPreviousImage();
    }
    );
...
MMGallery.showAmplifiedImage = function (source) {
    $(".MMFilesItemImage").each(function () {
        $("#magnifiedImage").attr("src", source);
        $("#fullscreenImageContainer").show();
    });
};

MMGallery.showNextImage = function () {
    this.currentFile++;
    while (this.files[this.currentFile].file_type_id != 1) {
        this.currentFile++;
    ...
    $("#magnifiedImage").attr("src", file.url);
};

MMGallery.showPreviousImage = function () {
    this.currentFile--;
...
MMGallery.setFile = function () {
    ...
    if (file.file_type_id == 1) {
        ...
        var dummyString = '<div id="' + randomId + '" style="opacity:1">';
        dummyString += '<iframe width="650" height="389" style="position: absolute; margin-top: 22px;" src="https://www.youtube.com/embed/' + file.url + '"</iframe></div>';

        var filesDiv = $('#MMFiles');
        var currentFileDiv = $(filesDiv.children()[0]);
        $('#MMFiles').html(dummyString);
    }
    var instance = this;
    Tools.delay(100,
            function () {
...
MMGallery.setThumbnails = function () {
    var html = '';
    for (var i = 0; i < this.files.length; i++) {
        var url = 'https://img.youtube.com/vi/' + this.files[i]['url'] + '/mqdefault.jpg';
        ...
        if (file.file_type_id == 1) {
            ...
            var rightMargin = i == this.files.length - 1 ? "" : "rightMarginItem";
            html += '<div id="MMGalleryItem' + (i + 1) + '" class="MMGalleryItemWrapper ' + rightMargin + '">';
            html += '<img class="MMGalleryImage" src="' + url + '">';
            html += '</div>';
        }
